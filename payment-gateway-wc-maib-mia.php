...
function maib_mia_init()
{
    ...
    class WC_Gateway_MAIB_MIA extends WC_Payment_Gateway
    {
         ...
         */
        private function maib_mia_get_response_result($response)
        {
            if (!empty($response)) {
        ...
        public function process_refund($order_id, $amount = null, $reason = '')
        {
            ...
            if (empty($pay_id)) {
                /* translators: 1: Order ID, 2: Meta field key */
                $message = esc_html(sprintf(__('Order #%1$s missing meta %2$s', 'payment-gateway-wc-maib-mia'), $order_id, self::MOD_PAY_ID));
